...
class DecoderCBatchNormNoResnet(nn.Module):
    ...
    def __init__(self, dim=3, z_dim=128, c_dim=128,
                 hidden_size=256, activation='sin'):
        super().__init__()
        self.z_dim = z_dim
        if not z_dim == 0:
            self.fc_z = nn.Linear(z_dim, hidden_size)
        ...
        self.fc_3 = nn.Conv1d(hidden_size + dim, hidden_size, 1)
        ...
        self.bn_3 = CBatchNorm1d(c_dim, hidden_size + dim)
        self.bn_4 = CBatchNorm1d(c_dim, hidden_size)
        self.bn_5 = CBatchNorm1d(c_dim, hidden_size)

        self.fc_out = nn.Conv1d(hidden_size, 3, 1)
        ...
        if activation == 'sin':
            ...
            self.actvn = lambda x: F.leaky_relu(x, 0.2) # torch.sin(x)
        else:
            self.actvn = F.elu
